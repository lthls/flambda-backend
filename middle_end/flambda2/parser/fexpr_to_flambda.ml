...
let unop env (unop : Fexpr.unop) : Flambda_primitive.unary_primitive =
  ...
  | Box_number bk -> Box_number (bk, Alloc_mode.With_region.must_be_heap)
...
let varop (varop : Fexpr.varop) n : Flambda_primitive.variadic_primitive =
  ...
  | Make_block (tag, mutability) ->
    ...
    in
    Make_block (kind, mutability, Alloc_mode.With_region.must_be_heap)
...
let set_of_closures env fun_decls value_slots =
  ...
  in
  Set_of_closures.create ~value_slots Alloc_mode.With_region.must_be_heap fun_decls
  ...
  | Apply
    ...
    let call_kind =
      ...
      | Function (Direct { code_id; function_slot = _ }) ->
        ...
        in
        Call_kind.direct_function_call code_id ~return_arity Alloc_mode.must_be_heap
      | Function Indirect -> (
        match arities with
        | Some { params_arity = Some params_arity; ret_arity } ->
          ...
          Call_kind.indirect_function_call_known_arity ~param_arity
            ~return_arity Alloc_mode.must_be_heap
        | None | Some { params_arity = None; ret_arity = _ } ->
          Call_kind.indirect_function_call_unknown_arity Alloc_mode.must_be_heap)
      | C_call { alloc } -> (
        match arities with
        | Some { params_arity = Some params_arity; ret_arity } ->
          let param_arity =
