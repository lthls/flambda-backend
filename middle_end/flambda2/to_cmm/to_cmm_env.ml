...
type t =
  { (* Global information.
    ...
    (* Cmm expressions (of the form [Cvar ...]) for all bound variables in scope. *)
...
let is_cmm_simple cmm =
  ...
  | Cconst_symbol _ | Cvar _ -> true
  | _ -> false)[@ocaml.warning "-4"]

let create_binding_aux (type a) ?extra env effs var
...
let create_binding (type a) ?extra env effs var
    ~(inline : a inline) (bound_expr : a bound_expr) =
    (* In order to avoid generating binding of the form: "let x = y in ...",
       when 'y' is trivial i.e. is a value that fits in a register,
       we mark 'x' as a must_inline_and_duplicate (since it basically replaces
       a variable by either another variable, a constant, or a symbol). *)
    match bound_expr with
    | Simple { cmm_expr; } when is_cmm_simple cmm_expr ->
      create_binding_aux ?extra env effs var ~inline:Must_inline_and_duplicate (Split { cmm_expr })
    | Simple _ | Split _ | Splittable _ ->
      create_binding_aux ?extra env effs var ~inline bound_expr

let bind_variable (type a) ?extra env var ~inline ~(defining_expr : a bound_expr)
  ...
  | May_inline_once | Must_inline_once | Do_not_inline -> (
    ...
    | Generative_duplicable ->
      begin match (inline : a inline) with
        | Must_inline_once ->
          (* CR: this allows to move allocations marked as `Must_inline_once`
             past function calls (and other
...
let bind_let_variable ?extra env var ~inline ~defining_expr
  ...
  | Must_inline_once ->
    ...
    let defining_expr = splittable_no_args name defining_expr effects_and_coeffects_of_defining_expr in
    bind_variable ?extra env var ~effects_and_coeffects_of_defining_expr
...
let split_complex_binding (binding : complex binding) =
  ...
  | Splittable { name = _; args; make_expr; } ->
    let (new_bindings, _), new_cmm_args =
      List.fold_left_map (fun (new_bindings, order) (cmm_arg, arg_effs) ->
          if is_cmm_simple cmm_arg then
            (new_bindings, order), cmm_arg
          else begin
            (* we need to rebind the argument *)
            let new_cmm_var =
              Backend_var.With_provenance.create
                ?provenance:None (Backend_var.create_local (Format.asprintf "split_tmp_%d" order))
            in
            let binding = Binding { order;
                                    effs = arg_effs;
                                    inline = Do_not_inline;
                                    bound_expr = Simple { cmm_expr = cmm_arg };
                                    cmm_var = new_cmm_var; } in
            (binding :: new_bindings, order - 1), C.var (Backend_var.With_provenance.var new_cmm_var)
          end
        ) ([], binding.order - 1) args
    in
    let new_cmm_expr, new_effs = make_expr new_cmm_args in
    begin match To_cmm_effects.classify_by_effects_and_coeffects new_effs with
      | Pure | Generative_duplicable -> ()
      | Effect | Coeffect_only ->
        Misc.fatal_errorf
          "Once split, a 'must_inline_once' binding cannot have \
          effects or coeffects, since it can be moved around to be inlined."
    end;
    let split_binding = {
      order = binding.order;
      effs = new_effs;
...
let split_and_inline env var binding =
  ...
  | Some (new_bindings, split_binding) ->
    let env =
      (* for duplicated bindings, we need to replace the original splittable binding
         with the new split binding in the bindings map of the env *)
      match split_binding.inline with
      | Must_inline_once -> env
      | Must_inline_and_duplicate ->
        { env with bindings = Variable.Map.add var (Binding split_binding) env.bindings; }
    in
    let env = List.fold_left (fun env new_binding ->
        let flambda_var = Variable.create "to_cmm_tmp" in
        { env with bindings = Variable.Map.add flambda_var new_binding env.bindings; }
      ) env new_bindings
    in
...
let pop_from_top_stage ?consider_inlining_effectful_expressions env var =
  ...
  | Coeffect_only vars_from_stage :: prev_stages ->
    ...
    if Variable.Set.mem var vars_from_stage then
...
let inline_variable ?consider_inlining_effectful_expressions env var =
  ...
  | exception Not_found ->
    (* this happens for continuation parameters and bindings that have been flushed *)
    begin match Variable.Map.find var env.vars with
      | exception Not_found ->
  ...
  | Binding binding ->
    begin match binding.inline with
      ...
      | Must_inline_once ->
        ...
        begin match To_cmm_effects.classify_by_effects_and_coeffects binding.effs with
          | Pure | Generative_duplicable ->
            will_inline_complex env binding
          | Effect | Coeffect_only ->
            begin match pop_from_top_stage ?consider_inlining_effectful_expressions env var with
...
let flush_delayed_lets ?(entering_loop = false) env =
  ...
  (* CR-someday mshinwell: work out a criterion for allowing substitutions into
     loops.
     CR gbury: this is now done by creating a binding with the inline status
     `Must_inline_and_duplicate`, so the caller of `to_cmm_env` has to make that
     decision of whether to substitute inside loops. *)
  let bindings_to_flush = ref M.empty in
  let flush (Binding b as binding) =
    if M.mem b.order !bindings_to_flush then
      Misc.fatal_errorf "Duplicate order for bindings when flushing";
    bindings_to_flush := M.add b.order binding !bindings_to_flush
  in
  let bindings_to_keep =
    Variable.Map.filter_map (fun _ (Binding b as binding) ->
        match b.inline with
        | Do_not_inline -> flush binding; None
        | Must_inline_and_duplicate ->
          begin match split_complex_binding b with
            | None -> (* already split *) Some binding
            | Some (arg_bindings, split_binding) ->
              List.iter flush arg_bindings; Some (Binding split_binding)
          end
        | Must_inline_once ->
          begin match To_cmm_effects.classify_by_effects_and_coeffects b.effs with
            (* when not entering a loop, and with pure/generative effects at most,
               we can wait to split the binding, so that we can have a chance to try
               and push the arguments down the branch (otherwise, when we split, the
               arguments of the splittable binding would be flushed before the branch
               in control flow). *)
            | (Pure | Generative_duplicable) when not entering_loop -> Some binding
        ...
        | May_inline_once ->
          begin match To_cmm_effects.classify_by_effects_and_coeffects b.effs with
            (* Unless entering a loop, we do not flush pure bindings that can be inlined,
               ensuring that the corresponding expressions are sunk down as far as possible,
               including past control flow branching points. *)
            | Pure -> if entering_loop then (flush binding; None) else Some binding
            | Generative_duplicable | Coeffect_only | Effect -> flush binding; None
          end
      ) env.bindings
  in
