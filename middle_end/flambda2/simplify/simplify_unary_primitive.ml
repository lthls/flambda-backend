...
let simplify_unbox_number (boxable_number_kind : K.Boxable_number.t) dacc
  ...
  let shape, result_kind =
    ...
    | Naked_float ->
      T.boxed_float_alias_to ~naked_float:result_var' (Alloc_mode.may_be_local ()), K.naked_float
    | Naked_int32 ->
      T.boxed_int32_alias_to ~naked_int32:result_var' (Alloc_mode.may_be_local ()), K.naked_int32
    | Naked_int64 ->
      T.boxed_int64_alias_to ~naked_int64:result_var' (Alloc_mode.may_be_local ()), K.naked_int64
    | Naked_nativeint ->
      ( T.boxed_nativeint_alias_to ~naked_nativeint:result_var' (Alloc_mode.may_be_local ()),
        K.naked_nativeint )
  in
  let alloc_mode =
  ...
  let dacc =
    ...
    | Unknown | Proved May_be_local -> dacc
    | Proved Must_be_heap ->
      DA.map_denv dacc ~f:(fun denv ->
          DE.add_cse denv
            (P.Eligible_for_cse.create_exn
               (Unary
                  ( Box_number (boxable_number_kind, Alloc_mode.With_region.must_be_heap),
...
let simplify_obj_dup dbg dacc ~original_term ~arg ~arg_ty ~result_var =
  ...
  | Proved (Tagged_immediate | Boxed (Must_be_heap, _, _)) -> elide_primitive ()
  | Proved (Boxed ((May_be_local), boxable_number, contents_ty)) -> (
    let boxer =
    ...
    in
    let ty = boxer contents_ty Alloc_mode.must_be_heap in
    let dacc = DA.add_variable dacc result_var ty in
    ...
    | contents ->
      SPR.create
        (Named.create_prim
           (Unary
              ( Box_number (boxable_number, Alloc_mode.With_region.must_be_heap),
                contents ))
           dbg)
        ~try_reify:true dacc)
  | Unknown -> (
    match T.prove_strings typing_env arg_ty with
    | Proved (Must_be_heap, _) -> elide_primitive ()
    | Proved (May_be_local, _) | Unknown ->
      SPR.create_unknown dacc ~result_var K.value ~original_term)
