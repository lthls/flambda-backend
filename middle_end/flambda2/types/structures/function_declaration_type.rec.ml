...
let meet (env : Meet_env.t) (t1 : t) (t2 : t)
  ...
  | Ok (Inlinable {
    ...
    let target_code_age_rel = TE.code_age_relation typing_env in
    let resolver = TE.code_age_relation_resolver typing_env in
    let check_other_things_and_return code_id
      : (t, TEE.t) Meet_result.t =
      let code_id_result : Code_id.t Meet_result.return_value =
        if Code_id.equal code_id1 code_id2 then Both_inputs
        else if Code_id.equal code_id code_id1 then Left_input
        else if Code_id.equal code_id code_id2 then Right_input
        else New_result code_id
      in
      begin match Type_grammar.meet env rec_info1 rec_info2 with
      | Bottom -> Bottom
      | Ok (rec_info_result, extension) ->
        let result =
          Meet_result.combine code_id_result code_id1 code_id2
            rec_info_result rec_info1 rec_info2
            (fun code_id rec_info : t ->
    ...
    with
    | Ok code_id ->
      check_other_things_and_return code_id
    | Bottom -> Bottom
    end
