...
let any_boxed_float = TG.box_float TG.any_naked_float (Alloc_mode.may_be_local ())

let any_boxed_int32 = TG.box_int32 TG.any_naked_int32 (Alloc_mode.may_be_local ())

let any_boxed_int64 = TG.box_int64 TG.any_naked_int64 (Alloc_mode.may_be_local ())

let any_boxed_nativeint = TG.box_nativeint TG.any_naked_nativeint (Alloc_mode.may_be_local ())
...
let closure_with_at_least_these_function_slots ~this_function_slot
  ...
  let by_function_slot =
    ...
    TG.Row_like_for_closures.create_at_least this_function_slot
      set_of_closures_contents closures_entry
  in
  TG.create_closures (Alloc_mode.may_be_local ()) by_function_slot
...
let rec unknown_with_subkind ?(alloc_mode = (Alloc_mode.may_be_local ()))
