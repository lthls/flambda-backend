...
and head_of_kind_value = private
  ...
  | Mutable_block of { alloc_mode : Alloc_mode.t }
  | Boxed_float of t * Alloc_mode.t
  | Boxed_int32 of t * Alloc_mode.t
  | Boxed_int64 of t * Alloc_mode.t
  | Boxed_nativeint of t * Alloc_mode.t
  | Closures of
...
val boxed_float_alias_to :
  naked_float:Variable.t -> Alloc_mode.t -> t

val boxed_int32_alias_to :
  naked_int32:Variable.t -> Alloc_mode.t -> t

val boxed_int64_alias_to :
  naked_int64:Variable.t -> Alloc_mode.t -> t

val boxed_nativeint_alias_to :
  naked_nativeint:Variable.t -> Alloc_mode.t -> t
...
val mutable_block : Alloc_mode.t -> t

val create_closures : Alloc_mode.t -> row_like_for_closures -> t
...
module Row_like_for_blocks : sig
  ...
  val create_blocks_with_these_tags :
    field_kind:Flambda_kind.t -> Tag.Set.t -> Alloc_mode.t -> t
  ...
  val get_singleton :
    t ->
    (Tag_and_size.t * Product.Int_indexed.t * Alloc_mode.t) option
...
module Head_of_kind_value : sig
  ...
  val create_mutable_block : Alloc_mode.t -> t

  (* CR-someday mshinwell: these alloc mode params should probably be
     labelled *)
  val create_boxed_float : flambda_type -> Alloc_mode.t -> t

  val create_boxed_int32 : flambda_type -> Alloc_mode.t -> t

  val create_boxed_int64 : flambda_type -> Alloc_mode.t -> t

  val create_boxed_nativeint : flambda_type -> Alloc_mode.t -> t

  val create_tagged_immediate : Targetint_31_63.t -> t

  val create_closures :
    Row_like_for_closures.t -> Alloc_mode.t -> t
