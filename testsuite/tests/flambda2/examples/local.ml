...
let return_local () = exclave_ [1; 2; 3]

let rec map_local ~f:(f : _ @ local) (l : _ @ local) =
  exclave_ (match l with [] -> [] | a :: l -> f a :: map_local ~f l)

let rec length (l : _ @ local) = match l with [] -> 0 | _ :: l -> 1 + length l
